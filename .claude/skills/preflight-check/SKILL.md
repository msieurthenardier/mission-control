...
| `migration-pending:{id}` | A methodology migration applies (legacy layout, outdated states, missing sections) | Needs migration via `/init-project` |
...
- **Needs full init**: projects with `missing`, `agent-crews:missing`, or pending migrations
...
1. **Run `/init-project`** using the skill workflow (read `.claude/skills/init-project/SKILL.md` and execute)
   ...
   - For projects with pending migrations: run from Step 2 (migrations)
