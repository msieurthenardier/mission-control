...
3. **Load prior maintenance reports (if any exist)**
...
1. **Count source files** — `find . -name "*.{ext}" -not -path "*/node_modules/*" -not -path "*/.git/*" | wc -l` (use appropriate extensions for the stack)
...
Read `.flightops/agent-crews/routine-maintenance.md` for crew definitions and prompts (fall back to defaults at `${SKILL_DIR}/../init-project/defaults/agent-crews/routine-maintenance.md`).

**Validate structure**: The crew file MUST contain `## Crew`, `## Interaction Protocol`, and `## Prompts` sections with fenced code blocks. If the file exists but is malformed, STOP and tell the user: "Crew file `routine-maintenance.md` is missing required sections. Either fix it manually or re-run `/mission-control:init-project` to reset to defaults."
...
This phase produces the mission and its flights, ready for `/mission-control:agentic-workflow` to design and execute the legs. It does not scaffold legs and does not run `/mission-control:mission` or `/mission-control:flight` separately.
...
- **Technical Approach**: Per finding, the fix strategy plus the Inspector's evidence and Architect's recommendation — enough for `/mission-control:agentic-workflow` to design legs. One finding maps to one leg.
...
This skill NEVER modifies source files, configuration, or dependencies in the project. The Inspector runs checks and reports findings. The only files created are the maintenance report and optionally a maintenance mission scaffold (mission and flights) — all are Flight Control artifacts.
