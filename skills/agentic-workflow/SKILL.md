...
Orchestrate multi-agent flight execution. You drive the full leg cycle — designing legs, spawning Developer and Reviewer agents, and managing git workflow — for the project's flight. Leg design review is risk-tiered (high-risk legs get a per-leg design review; low-risk legs go straight to implementation), and code review and commit are deferred until after the last autonomous leg completes. This eliminates per-leg review/commit overhead while keeping the same leg design and implementation structure.
...
1. **Read `.flightops/ARTIFACTS.md`** for how this project handles each artifact — storage location, format, and any actions the project defines at create and transition time (e.g., transitioning a ticket, posting a notification). Honor these when you or a spawned agent moves an artifact through its lifecycle.
...
A flight (or a specific leg) may declare its acceptance criteria via a **behavior test** spec — a Zephyr-style two-column Action | Expected Result table, run via the `/mission-control:behavior-test` skill with two live AI agents (Executor + Validator) using the Witnessed pattern. Behavior tests verify real-environment observation that doesn't fit unit/integration tests (UI flows, multi-component interactions, AI agent behavior).

**Where they fit in this workflow:**

- A leg authored during Phase 2a may reference a behavior-test slug instead of (or in addition to) inline verification steps — e.g., "Acceptance: `/mission-control:behavior-test discord-engagement` passes."
- When the Flight Director reaches such a leg, run the test by invoking `/mission-control:behavior-test {slug}` directly (not by spawning a Developer agent — the run skill orchestrates its own crew).
...
**Authoring behavior-test specs**: specs are written inline during planning conversations (flight design, leg design), not via a dedicated skill. See `${SKILL_DIR}/../behavior-test/AUTHORING.md` for the authoring guide (interview shape, spec format, common pitfalls). Format is canonical in the project's `.flightops/ARTIFACTS.md`.

**Crew prompts** (Executor + Validator) live at `.flightops/agent-crews/behavior-tests-execution.md` — installed by `/mission-control:init-project` and modifiable per project.

## Architecture

The Flight Director (you) orchestrates according to this skill. Project crew composition, roles, models, and prompts are defined in `.flightops/agent-crews/leg-execution.md`.

**Separation is mandatory.** Project crew agents run in the project and load its CLAUDE.md and conventions. The Reviewer has no knowledge of the Developer's reasoning — only the resulting changes. This provides objective review.
