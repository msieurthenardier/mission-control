...
A behavior test spec is a markdown file that defines what the system should do (Actions) and how to tell that it did (Expected Results), in a two-column table that a human could execute by reading top-to-bottom. The `/mission-control:behavior-test` skill runs the spec with two live AI agents — an Executor that performs the Actions and an independent Validator that judges the Expected Results.

**Specs are authored inline during planning conversations** — there is no `/mission-control:behavior-test author` command. When the operator and the Orchestrator are designing a flight, leg, mission, debrief, or maintenance finding and identify a verification need that warrants a behavior test, the Orchestrator drafts the spec file directly using the format below. This guide is what the Orchestrator consults to do that well.

The canonical spec format lives in `.flightops/ARTIFACTS.md` under "Behavior Test — Spec". This guide covers the *process* of writing one well — when to author, what questions to ask, what mistakes to avoid.
...
|---|---|
| `/mission-control:flight` planning | The flight's verification approach needs real-environment observation that a unit test can't provide. |
| Leg design (during `/mission-control:agentic-workflow`) | A leg's acceptance criterion can only be verified by acting on the running system. |
| `/mission-control:mission` planning | The mission's success criteria include behaviors that span multiple components and surfaces. |
| `/mission-control:routine-maintenance` | A finding is worth a regression gate — make the test now so future flights can re-run it. |
| `/mission-control:flight-debrief` / `/mission-control:mission-debrief` | A debrief surfaces "we should have caught this with a test" — author it as a follow-up action. |
...
The format lives in `.flightops/ARTIFACTS.md` under "Behavior Test — Spec". A skeleton:
...
**Unaudited apparatus premises.** A spec that names concrete apparatus mechanics ("call X via tool Y", "the response is shaped Z", "identity A can perform op B") is making empirical claims about the system under test — and a wrong one burns an expensive multi-agent live run at its first step. Before a new spec's FIRST run, have a Developer agent trace every such premise against the actual code (authorization tiers on each tool, exact return shapes, where auth is enforced relative to routing). Treat the spec as a design artifact: premise-audit it the way a leg spec gets design-reviewed. Corrections to a `draft` spec before its first pass are authoring, not drift.

**Live-session fixtures decay (idle pruning).** When the fixture is "the same live session across N steps" (an auth/session-semantics test), remember servers commonly prune idle sessions — a fixture session lost mid-run turns a crisp auth assertion into an ambiguous session-routing error. Mitigations that proved out: keep inter-step latency low; hold the live transports in a **long-lived daemon process** (per-step calls reach the daemon over a local control port while the sessions live on in-process — this also makes "same session, no reconnect" mechanically provable via stable session ids, letting the Validator verify continuity as fact rather than executor assertion); and spec the expected rejection SHAPE precisely enough to discriminate the failure you're testing from the pruning confound (e.g. bare 401 at a pre-routing auth gate vs 404-with-body from session routing). Apparatus-only identities lost to pruning can simply reconnect; fixture identities cannot.
...
3. Reference the spec slug in the parent artifact's acceptance criteria — e.g., a leg artifact may say "Run `/mission-control:behavior-test <slug>` to verify acceptance".
