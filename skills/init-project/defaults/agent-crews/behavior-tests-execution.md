...
`/mission-control:behavior-test {slug}` skill. Specs are authored inline during planning
conversations (flight, leg, mission, debrief, maintenance) rather than via
a dedicated authoring skill — see the authoring guide (`AUTHORING.md`) shipped with the
mission-control plugin's behavior-test skill.
...
### Executor
- **Context**: project root (current working directory) — loads the project's CLAUDE.md.
